...
curl $curl_opts http://${INGRESS_NLB_IP} > /dev/null
curl $curl_opts --insecure https://${INGRESS_NLB_IP} > /dev/null
...
## Before handing out to the cleanup phase, delete the ingress controller Service
## in order to delete the associated NLB, otherwise it won't be possible to delete the
