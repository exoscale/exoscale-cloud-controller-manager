...
_until_success "test \$(kubectl get nodes --no-headers -l '!node-role.kubernetes.io/control-plane' | grep '^test-\S*-pool-' | wc -l) -ge 2"

if [ "$TARGET_CLUSTER" != "sks" ]; then
	_until_success "test \$(kubectl get nodes --no-headers -l '!node-role.kubernetes.io/control-plane' | grep '^test-\S*-external\s' | wc -l) -ge 1"
fi
